...
type street = { park_count : int; homes : house array; fences : int list } [@@deriving show, yojson]
...
type effect =
  ...
  | BPR [@@deriving show, yojson]

type home_assignment = { street_num : int; house_num : int; value : int } [@@deriving show, yojson]
...
type game_error =
  ...
  | PoolAlreadyExists
  | TooManyParks
  | InvalidAction of string [@@deriving show, yojson]
...
type estate_plan = { claimaint_count : int; establishments : int list } [@@deriving show, yojson]
