...
module Model = struct
  ...
  let add_new t =
    { counters = Map.set t.counters ~key:(Map.length t.counters) ~data:0 }
  ...
  let update t ~pos ~diff =
    ...
    | None -> t
    | Some old_val ->
        { counters = Map.set t.counters ~key:pos ~data:(old_val + diff) }
...
module Action = struct
  type t = New_counter | Update of { pos : int; diff : int } [@@deriving sexp]
end
